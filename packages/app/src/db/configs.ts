export const DefaultConfigs: {
    key: string
    value: string
    description?: string
}[] = [
    {
        key: "llm.models",
        value: JSON.stringify([
            "gpt-5.2",
            "gpt-5.1",
            "gpt-5",
            "gpt-5-mini",
            "gpt-5-nano",
            "gpt-5.2-chat-latest",
            "gpt-5.1-chat-latest",
            "gpt-5-chat-latest",
            "gpt-5.2-codex",
            "gpt-5.1-codex-max",
            "gpt-5.1-codex",
            "gpt-5-codex",
            "gpt-5.2-pro",
            "gpt-5-pro",
            "gpt-4.1",
            "gpt-4.1-mini",
            "gpt-4.1-nano",
            "gpt-4o",
            "gpt-4o-2024-05-13",
            "gpt-4o-mini",
        ]),
        description: "Available global LLM model candidates shown in selectors (JSON array).",
    },
    {
        key: "search.intent_resolve.model",
        value: "gpt-5-mini",
        description: "LLM model used for search intent resolution.",
    },
    {
        key: "search.content_generation.model",
        value: "gpt-5.2-chat-latest",
        description: "LLM model used for search content/article generation.",
    },
    {
        key: "search.spelling_correction.model",
        value: "gpt-5-mini",
        description: "LLM model used for search spelling correction.",
    },
    {
        key: "search.spelling_correction.mode",
        value: "auto",
        description: "Spell correction mode for search query normalization: off|auto|force.",
    },
    {
        key: "search.filetype.allowlist",
        value: JSON.stringify([
            "md",
            "txt",
            "sh",
            "bash",
            "zsh",
            "py",
            "js",
            "ts",
            "tsx",
            "jsx",
            "json",
            "yaml",
            "yml",
            "toml",
            "ini",
            "xml",
            "sql",
            "csv",
            "java",
            "c",
            "cpp",
            "h",
            "hpp",
            "go",
            "rs",
            "rb",
            "php",
        ]),
        description: "Allowed filetype extensions for search output (JSON array).",
    },
    {
        key: "llm.base_url",
        value: "",
        description: "Optional OpenAI-compatible base URL used by LLM calls. Leave empty for default OpenAI endpoint.",
    },
    {
        key: "search.example_queries",
        value: JSON.stringify([
            "quicksort implementation filetype:py",
            "ubuntu install ohmyzsh filetype:sh",
            "sqlite fts5 bm25",
            "transformer attention matrix",
            "batch normalization",
            "policy gradient",
            "bellman equation",
            "backpropagation chain rule derivation",
            "laplace transform table",
            "insulin resistance basics",
            "ww2 causes",
            "nationalism europe 19th century",
            "ww2 timeline",
            "versailles treaty impact ww2",
            "fascism rise italy",
            "weimar collapse causes",
            "imperial japan expansion",
            "interwar europe politics",
            "post ww2 decolonization",
            "cold war origins"
        ]),
        description: "Example queries shown on the search home screen for first-time users (JSON array).",
    },
    {
        key: "search.example_queries.zh-cn",
        value: JSON.stringify([
            "python 快速排序 filetype:py",
            "ubuntu 安装 ohmyzsh filetype:sh",
            "sqlite fts5 bm25",
            "transformer attention matrix",
            "batch normalization",
            "policy gradient",
            "bellman equation",
            "反向传播 链式法则 推导",
            "拉普拉斯变换 表",
            "胰岛素抵抗 基础概念",
        ]),
        description: "Localized example queries for Simplified Chinese users (JSON array).",
    },
    {
        key: "search.example_queries.zh-tw",
        value: JSON.stringify([
            "python 快速排序 filetype:py",
            "ubuntu 安裝 ohmyzsh filetype:sh",
            "sqlite fts5 bm25",
            "transformer attention matrix",
            "batch normalization",
            "policy gradient",
            "bellman equation",
            "反向傳播 鏈式法則 推導",
            "拉普拉斯轉換 表",
            "胰島素阻抗 基礎概念",
        ]),
        description: "Localized example queries for Traditional Chinese users (JSON array).",
    },
    {
        key: "search.example_queries.ja",
        value: JSON.stringify([
            "python クイックソート filetype:py",
            "ubuntu ohmyzsh インストール filetype:sh",
            "sqlite fts5 bm25",
            "transformer attention matrix",
            "batch normalization",
            "policy gradient",
            "bellman equation",
            "バックプロパゲーション 連鎖律 導出",
            "ラプラス変換 表",
            "インスリン抵抗性 基礎",
        ]),
        description: "Localized example queries for Japanese users (JSON array).",
    },
    {
        key: "search.example_queries.ko",
        value: JSON.stringify([
            "python 퀵소트 filetype:py",
            "ubuntu ohmyzsh 설치 filetype:sh",
            "sqlite fts5 bm25",
            "transformer attention matrix",
            "batch normalization",
            "policy gradient",
            "bellman equation",
            "역전파 연쇄법칙 유도",
            "라플라스 변환 표",
            "인슐린 저항성 기초",
        ]),
        description: "Localized example queries for Korean users (JSON array).",
    },
    {
        key: "search.example_queries.es",
        value: JSON.stringify([
            "quicksort en python filetype:py",
            "ubuntu instalar ohmyzsh filetype:sh",
            "sqlite fts5 bm25",
            "transformer attention matrix",
            "batch normalization",
            "policy gradient",
            "bellman equation",
            "derivacion backpropagation regla de la cadena",
            "tabla transformada de laplace",
            "resistencia a la insulina conceptos basicos",
        ]),
        description: "Localized example queries for Spanish users (JSON array).",
    },
    {
        key: "search.example_queries.fr",
        value: JSON.stringify([
            "quicksort python filetype:py",
            "ubuntu installer ohmyzsh filetype:sh",
            "sqlite fts5 bm25",
            "transformer attention matrix",
            "batch normalization",
            "policy gradient",
            "bellman equation",
            "derivation backpropagation regle de la chaine",
            "table de transformee de laplace",
            "resistance a l insuline notions de base",
        ]),
        description: "Localized example queries for French users (JSON array).",
    },
    {
        key: "search.example_queries.de",
        value: JSON.stringify([
            "quicksort python filetype:py",
            "ubuntu ohmyzsh installieren filetype:sh",
            "sqlite fts5 bm25",
            "transformer attention matrix",
            "batch normalization",
            "policy gradient",
            "bellman equation",
            "backpropagation kettenregel herleitung",
            "laplace transformation tabelle",
            "insulinresistenz grundlagen",
        ]),
        description: "Localized example queries for German users (JSON array).",
    },
    {
        key: "search.recent.min_query_chars",
        value: "3",
        description: "Minimum query length required for storing search history.",
    },
    {
        key: "search.recent.dedupe_window_seconds",
        value: "300",
        description: "Skip recording duplicate recent queries within this time window (in seconds).",
    },
    {
        key: "search.recent.blacklist_terms",
        value: JSON.stringify(["test", "testing", "asdf", "qwer", "zxcv", "1234"]),
        description: "Exact query terms excluded from recent history chips (JSON array, case-insensitive).",
    },
    {
        key: "llm.api_key.env_name",
        value: "OPENAI_API_KEY",
        description: "Environment variable name used to read API key for LLM calls.",
    },
    {
        key: "llm.tool_choice.mode",
        value: "force",
        description: "Tool-call selection strategy for structured LLM requests: force|auto.",
    },
    {
        key: "llm.retry.max_attempts",
        value: "2",
        description: "Maximum attempts for LLM generation calls before giving up.",
    },
    {
        key: "llm.request.timeout_ms",
        value: "100000",
        description: "Per-request timeout in milliseconds for LLM generation calls.",
    },
    {
        key: "search.article_generation_eta.enabled",
        value: "1",
        description: "Enable estimated article-generation timing hints on search UI: 1|0.",
    },
    {
        key: "search.article_generation_eta.sample_size",
        value: "20",
        description: "How many recent completed article-generation runs to average for ETA.",
    },
]
