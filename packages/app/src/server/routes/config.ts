...
import { getPresetValues, parsePresetName } from "../../configPreset.js"
...
export function createConfigRouter(ctx: AppCtx) {
    ...
    router.get("/preset/:preset", (req, res) => {
        const preset = parsePresetName(req.params.preset?.trim())
        if (!preset) {
            res.status(400).json({ error: "invalid preset" })
            return
        ...
        const targetValues = getPresetValues(preset)
        const changes = Object.entries(targetValues).map(([key, nextValue]) => {
            const currentValue = configDB.getValue(key) ?? ""
            return {
                key,
                currentValue,
                nextValue,
