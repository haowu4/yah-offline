...
function inferDevAttachments(userInput: string): MailLLMReply["attachments"] {
  ...
  if (
    ...
    const base = slugify(userInput).slice(0, 28) || "mail-notes"
    attachments.push({
      kind: "text",
      filename: `${base}.txt`,
      modelQuality: "normal",
      content:
        `Development-mode generated notes for: ${userInput}\n` +
  ...
  if (
    ...
    const base = slugify(userInput).slice(0, 28) || "mail-image"
    attachments.push({
      kind: "image",
      filename: `${base}.png`,
      modelQuality: "normal",
      prompt: `Simple illustration for: ${userInput}`,
    })
  }
