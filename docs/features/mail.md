...
- Context: each thread has one summary context plus sliding window.

### Context Management

We use a hybrid approach, system prompt + summary + sliding window.
...
- thread page `/mail/thread/:threadId`, this page looks similar to viewing a email thread. user can click each mail to open it. but by default it just show a mail title. and user can post new message on this page. (also can provide which model to use)
